...
public class DepartmentServiceImpl implements DepartmentService {
	...
	public ResponseEntity<List<DepartmentWrapper>> getAllActiveDepartment() {
		...
		try {
			if (jwtFilter.isAdmin()) {
				return new ResponseEntity<List<DepartmentWrapper>>(departmentDao.getAllActiveDepartments(),
						HttpStatus.OK);
			} else {
				return new ResponseEntity<List<DepartmentWrapper>>(list, HttpStatus.UNAUTHORIZED);
			}
	...
	public ResponseEntity<List<DepartmentWrapper>> getDepartmentInfo(Integer id) {
		...
		try {
			if (jwtFilter.isAdmin()) {
				...
				if (optional.isPresent()) {
					return new ResponseEntity<List<DepartmentWrapper>>(departmentDao.getDepartmentById(id),
							HttpStatus.OK);
				} else {
					return new ResponseEntity<List<DepartmentWrapper>>(list, HttpStatus.UNAUTHORIZED);
				}
